...
in pythonPackages // (rec {
     ...
     astroid = mkPython (rec {
       ...
       propDeps     = with pkgs; [ logilab_common six ];
     ...
     piazza-api = mkPython (rec {
       ...
       propDeps     = with pkgs; [ blockdiag docutils reportlab
     ...
     openpyxl_2_2_0-b1 = mkPython (rec {
       ...
       buildDeps    = with pkgs; [ pytest ];
       propDeps     = with pkgs; [ jdcal et_xmlfile ];
